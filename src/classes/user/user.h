...
public:
    User(std::string user_name="empty", std::string email="empty", std::string password="empty", bool admin_status=0, bool blocked_status=0)
    {user_name_ = user_name; email_ = email; password_ = password; admin_status_ = admin_status; blocked_status_ = blocked_status;}
    ...
    void SetBlocked(bool blocked_status){blocked_status_ = blocked_status;}
};
